...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!darkSendSigner.GetKeysFromSecret(activeMasternode.strMasterNodePrivKey, errorMessage, keyMasternode, pubKeyMasternode))
        {
            std::string notCapableReason = "Error upon calling SetKey: " + errorMessage + " - " + activeMasternode.strMasterNodePrivKey; 
